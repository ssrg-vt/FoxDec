...
import qualified Generic.Program as Program
...
--         else putStrLn $ "File: " ++ show (head args) ++ " does not exist."
--   where
--     read_address str = do
--       if take 2 str == "0x"
--         then return $ readHex' $ drop 2 str
--         else do
--           putStrLn usage_msg
...
--       Predicate q _ = postcondition
--   in filter (is_modified_register p q) (M.keys q)
--   where
