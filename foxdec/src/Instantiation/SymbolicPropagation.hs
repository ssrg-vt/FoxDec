...
mnemonic_to_semantics SAR si si'     = Apply $ sar
 where
  sar [a,SE_Immediate i] = SE_Op Sdiv si [a,SE_Immediate $ 2^i]
  sar [a,b]              = SE_Op Sar si [a,b]


mnemonic_to_semantics DIV_LO si si'  = Apply $ SE_Op UdivLo si
...
try_relocated_pointer fctxt a =
  case IM.lookup (fromIntegral a) $ ctxt_symbol_table ctxt of
    Just (PointerToLabel f _)  -> Just f
    Just (PointerToObject f _) -> Just f
    _ -> Nothing
 where
  ctxt = f_ctxt fctxt
 ...
 where
  ...
  try_symbol a =
    case IM.lookup (fromIntegral a) $ ctxt_symbol_table $ f_ctxt fctxt of
      Just (AddressOfLabel f _) -> Just $ External f
