...
Description : Symbolic execution of sequential lists of instructions of type @"X86.Instruction"@ on predicates of type @"Predicate"@.
-}
...
import Control.Monad.Extra (anyM,whenM,mapMaybeM,forM_)
...
transpose_fw_e ctxt p   (SE_Op op si es)      = SE_Op op si <$> mapM (transpose_fw_e ctxt p) es
 ...
 where
  ...
  mk_finit_entry (sp,v) =
    if is_immediate_pointer v then
      Just (sp,v)
    else case fromJust $ get_pointer_domain ctxt v of
...
--   *[RSP0-40,8] == 10
transpose_bw :: FContext -> String -> Word64 -> Predicate -> (StatePart, SimpleExpr) -> Maybe (StatePart, SimpleExpr)
transpose_bw ctxt f a p (sp,v) =
  let sp' = transpose_bw_sp ctxt p sp
    ...
    if is_invalid sp' then
      Nothing -- Left $ SourcelessMemWrite $ MemWriteFunction f a sp
    else
      Just (sp',v')
 where
...
transpose_bw_e :: FContext -> Predicate -> SimpleExpr -> SimpleExpr
...
transpose_bw_e ctxt p (SE_Op op si es)                 = SE_Op op si $ map (transpose_bw_e ctxt p) es
...
function_semantics :: FContext -> X86.Instruction -> String -> State Predicate Bool
...
function_semantics ctxt i f                      =
  if f `elem` functions_returning_bottom then do  -- and exiting function calls?
    ...
    write_reg ctxt RAX $ SE_Malloc (Just (X86.addressof i)) (Just "")
    return True
  else
    return False
...
call :: FContext -> X86.Instruction -> State Predicate ()
call ctxt i = do
  let f  = function_name_of_entry (f_ctxt ctxt) (f_entry ctxt)
  let f' = function_name_of_instruction (f_ctxt ctxt) i
  let i_a = X86.addressof i
  known <- function_semantics ctxt i $ takeUntilString "@GLIBC" f'

  rsp0 <- read_reg ctxt RSP 
  when (not known) $ do
    sub ctxt i_a (Storage RSP) (Immediate 8)
    p@(Predicate p_eqs _) <- get
    ...
    if postconditions == [] || any ((==) Nothing) postconditions then do
      ...
      forM_ return_registers (\r -> write_reg ctxt r $ Bottom $ FromCall $ f') -- TODO flags
    else do
      ...
      let q_eqs_transposed        = catMaybes $ map (transpose_bw ctxt f' i_a p) $ filter (uncurry do_transfer) $ M.toList q_eqs
      put $ Predicate M.empty None
      mapM_ (write_sp ctxt) q_eqs_transposed

      -- 2.) transfer stateparts that must be kept intact, and generation verification conditions if necessary
      q_transposed@(Predicate q_eqs _) <- get
      sps <- S.unions <$> (mapM (transfer_current_statepart f' i p q_transposed False) $ M.toList p_eqs)
      return ()
      --when (not $ S.null sps) $ modify $ add_function_constraint f' i_a params sps
  forced_insert_sp (SP_Reg RSP) rsp0
 where
  ...
  transfer_current_statepart f' i p@(Predicate p_eqs _) q@(Predicate q_eqs _) is_external (sp,v) = do
    if sp == SP_Reg RIP then do
      ...
      forced_insert_sp sp (SE_Immediate $ X86.addressof i + (fromIntegral $ sizeof i))
      return S.empty
      ...
      return S.empty
    else let v' = evalState (read_sp ctxt sp) q in
      if sp == SP_Reg RSP then do
        ...
        Nothing     -> do
          ...
          write_sp ctxt (sp,join_expr ctxt v v')
          return S.empty
  ...
  forced_insert_sp sp v = do
    p@(Predicate p_eqs flg) <- get
    put $ Predicate (M.insert sp v p_eqs) flg
  ...
  is_local_to_not_f (Src_StackPointer f') = function_name_of_entry (f_ctxt ctxt) (f_entry ctxt) /= f' -- TODO keep when f' is from current SCC callgraph
...
push :: FContext -> Word64 -> X86.Operand -> State Predicate ()
push ctxt i_a (Immediate imm) = do
  ...
  write_mem ctxt e1 si (SE_Immediate imm)
push ctxt i_a op1 = do
  ...
  e1 <- resolve_address ctxt address
  write_reg ctxt RSP e1
  write_mem ctxt e1 si e0

pop :: FContext -> Word64 -> X86.Operand -> State Predicate ()
pop ctxt i_a op1 = do
  let si = sizeof op1
...
lea :: FContext -> Word64 -> X86.Operand -> X86.Operand -> State Predicate ()
lea ctxt i_a op1 (EffectiveAddress a) = do
  e <- resolve_address ctxt a
  write_operand ctxt op1 e


leave :: FContext -> Word64 -> State Predicate ()
...
jmp ctxt i =
  if jump_is_actually_a_call (f_ctxt ctxt) i then
    -- A jump to an external symbol is treated as a function call and implicit RET
    call ctxt i >> ret ctxt (fromIntegral $ X86.addressof i)
  else
    return ()



write_flags :: (X86.Operand -> X86.Operand -> FlagStatus) -> X86.Operand -> X86.Operand -> State Predicate ()
write_flags g op1 op2 = do
  Predicate eqs flg <- get
  put $ Predicate eqs (g op1 op2)


mov_with_func_op2_to_op1 :: FContext -> Word64 -> (SimpleExpr -> SimpleExpr) -> X86.Operand -> X86.Operand -> State Predicate ()
...
mov_with_func1 :: FContext -> Word64 -> (FContext -> [SimpleExpr] -> SimpleExpr) -> Bool -> X86.Operand -> State Predicate ()
mov_with_func1 ctxt i_a f do_write_flags op1 = do
  e0 <- read_operand ctxt op1
  write_operand ctxt op1 (f ctxt [e0])
  when do_write_flags (write_flags (\_ _ -> None) op1 op1)

mov_with_func :: FContext -> Word64 -> (FContext -> [SimpleExpr] -> SimpleExpr) -> Bool -> X86.Operand -> X86.Operand -> State Predicate ()
mov_with_func ctxt i_a f do_write_flags op1 op2 = do
  e0 <- read_operand ctxt op1
  e1 <- read_operand ctxt op2
  write_operand ctxt op1 (f ctxt [e0,e1])
  when do_write_flags (write_flags (\_ _ -> None) op1 op2)

mov_with_func3 :: FContext -> Word64 -> (FContext -> [SimpleExpr] -> SimpleExpr) -> Bool -> X86.Operand -> X86.Operand -> X86.Operand -> State Predicate ()
mov_with_func3 ctxt i_a f do_write_flags op1 op2 op3 = do
  e0 <- read_operand ctxt op1
  e1 <- read_operand ctxt op2
  e2 <- read_operand ctxt op3
  write_operand ctxt op1 (f ctxt [e0,e1,e2])
...
movsx ctxt i_a op1 op2 = do
  e2 <- read_operand ctxt op2
  write_operand ctxt op1 (SE_SExtend (8 * sizeof op2) (8 * sizeof op1) e2)
...
cmov ctxt i_a op1 op2 = do
  e0 <- read_operand ctxt op1
  e1 <- read_operand ctxt op2
  write_operand ctxt op1 $ join_exprs ("cmov") ctxt [e0,e1]

xchg :: FContext -> Word64 -> X86.Operand -> X86.Operand -> State Predicate ()
...
inc :: FContext -> Word64 -> X86.Operand -> State Predicate ()
inc ctxt i_a op1 = do
  e1 <- read_operand ctxt op1
  write_operand ctxt op1 (SE_Op Plus (8 * sizeof op1) [e1,SE_Immediate 1])
  write_flags (\_ _ -> None) op1 op1

dec :: FContext -> Word64 -> X86.Operand -> State Predicate ()
dec ctxt i_a op1 = do
  e1 <- read_operand ctxt op1
  write_operand ctxt op1 (SE_Op Minus (8 * sizeof op1) [e1,SE_Immediate 1])
  write_flags (\_ _ -> None) op1 op1

or' :: FContext -> Word64 -> X86.Operand -> X86.Operand -> State Predicate ()
or' ctxt i_a op1 op2 = do
  e1 <- read_operand ctxt op1
  e2 <- read_operand ctxt op2
  write_operand ctxt op1 (SE_Op (Or) (8 * sizeof op1) [e1,e2])
  write_flags (\_ _ -> None) op1 op2

and' :: FContext -> Word64 -> X86.Operand -> X86.Operand -> State Predicate ()
and' ctxt i_a op1 op2 = do
  e1 <- read_operand ctxt op1
  e2 <- read_operand ctxt op2
  write_operand ctxt op1 (SE_Op (And) (8 * sizeof op1) [e1,e2])
  write_flags (\_ _ -> None) op1 op2


not' :: FContext -> Word64 -> X86.Operand -> State Predicate ()
not' ctxt i_a op1 = do
  e1 <- read_operand ctxt op1
  write_operand ctxt op1 (SE_Op (Not) (8 * sizeof op1) [e1])
  write_flags (\_ _ -> None) op1 op1
...
xor ctxt i_a op1 op2 = do
  if op1 == op2 then
    ...
    e1 <- read_operand ctxt op1
    e2 <- read_operand ctxt op2
    write_operand ctxt op1 (SE_Op (Xor) (8 * sizeof op1) [e1,e2])
    write_flags (\_ _ -> None) op1 op2
...
bt :: FContext -> Word64 -> X86.Operand -> X86.Operand -> State Predicate ()
...
bsr ctxt i_a op1 op2 = do
  e2 <- read_operand ctxt op2
  write_operand ctxt op1 $ SE_Op (Bsr) (8 * sizeof op1) [e2]
  write_flags (\_ _ -> None) op1 op2
...
div1 :: FContext -> Word64 -> X86.Operand -> State Predicate ()
div1 ctxt i_a op1 = do
  ...
  write_operand ctxt (Storage $ srcs !! 0) $ SE_Op (UdivHi) (8 * sizeof op1) [src0,src1,e1]
  write_operand ctxt (Storage $ srcs !! 1) $ SE_Op (UdivLo) (8 * sizeof op1) [src0,src1,e1]
  write_flags (\_ _ -> None) op1 op1


cdq :: FContext -> Word64 -> State Predicate ()
cdq ctxt i_a = do
  e1 <- read_operand ctxt (Storage EAX)
  write_operand ctxt (Storage EDX) (mk_bottom ctxt [e1])

cqo :: FContext -> Word64 -> State Predicate ()
cqo ctxt i_a = do
  e1 <- read_operand ctxt (Storage RAX)
  write_operand ctxt (Storage RDX) (mk_bottom ctxt [e1])

cdqe :: FContext -> Word64 -> State Predicate ()
cdqe ctxt i_a = do
  e1 <- read_operand ctxt (Storage EAX)
  write_operand ctxt (Storage RAX) (SE_SExtend 32 64 e1)
...
shl :: FContext -> Word64 -> X86.Operand -> X86.Operand -> State Predicate ()
shl ctxt i_a op1 op2@(Immediate i) = do
  e1 <- read_operand ctxt op1
  write_operand ctxt op1 (SE_Op (Times) (8 * sizeof op1) [e1,SE_Immediate $ 2^i])
  write_flags (\_ _ -> None) op1 op2
shl ctxt i_a op1 op2 = do
  e1 <- read_operand ctxt op1
  e2 <- read_operand ctxt op2
  write_operand ctxt op1 (SE_Op (Shl) (8 * sizeof op1) [e1,e2])
  write_flags (\_ _ -> None) op1 op2

shr :: FContext -> Word64 -> X86.Operand -> X86.Operand -> State Predicate ()
shr ctxt i_a op1 op2@(Immediate i) = do
  e1 <- read_operand ctxt op1
  write_operand ctxt op1 (SE_Op (Udiv) (8 * sizeof op1) [e1,SE_Immediate $ 2^i])
  write_flags (\_ _ -> None) op1 op2
shr ctxt i_a op1 op2 = do
  e1 <- read_operand ctxt op1
  e2 <- read_operand ctxt op2
  write_operand ctxt op1 (SE_Op Shr (8 * sizeof op1) [e1,e2])
  write_flags (\_ _ -> None) op1 op2

sar ctxt i_a op1 op2 = do
  e1 <- read_operand ctxt op1
  e2 <- read_operand ctxt op2
  write_operand ctxt op1 (SE_Op Sar (8 * sizeof op1) [e1,e2])
  write_flags (\_ _ -> None) op1 op2
...
rol :: FContext -> Word64 -> X86.Operand -> X86.Operand -> State Predicate ()
rol ctxt i_a op1 op2@(Immediate i) = do
  e1 <- read_operand ctxt op1
  write_operand ctxt op1 $ SE_Op Rol (8 * sizeof op1) [e1,SE_Immediate $ 2^i]
  write_flags (\_ _ -> None) op1 op2
rol ctxt i_a op1 op2 = mov_with_func ctxt i_a mk_bottom True op1 op2

ror :: FContext -> Word64 -> X86.Operand -> X86.Operand -> State Predicate ()
ror ctxt i_a op1 op2@(Immediate i) = do
  e1 <- read_operand ctxt op1
  write_operand ctxt op1 $ SE_Op Ror (8 * sizeof op1) [e1,SE_Immediate $ 2^i]
  write_flags (\_ _ -> None) op1 op2
ror ctxt i_a op1 op2 = mov_with_func ctxt i_a mk_bottom True op1 op2

{-
adc :: FContext -> X86.Operand -> X86.Operand -> State Predicate ()
adc ctxt op1 op2 = do
  e1 <- read_operand ctxt op1
  e2 <- read_operand ctxt op2
...
sbb :: FContext -> X86.Operand -> X86.Operand -> State Predicate ()
sbb ctxt op1 op2 = do
  e1 <- read_operand ctxt op1
  e2 <- read_operand ctxt op2
...
imul1 :: FContext -> Word64 -> X86.Operand -> State Predicate ()
imul1 ctxt i_a op1 = do
  ...
  write_operand ctxt (Storage $ srcs !! 0) $ mk_bottom ctxt [src1,e1] -- high part of multiplication
  write_operand ctxt (Storage $ srcs !! 1) $ SE_Op Times (8 * sizeof op1) [src1,e1]
  write_flags (\_ _ -> None) op1 op1

imul2 :: FContext -> Word64 -> X86.Operand -> X86.Operand -> State Predicate ()
imul2 ctxt i_a op1 op2 = do
  e1 <- read_operand ctxt op1
  e2 <- read_operand ctxt op2
  write_operand ctxt op1 (SE_Op Times (8 * sizeof op1) [e1,e2])
  write_flags (\_ _ -> None) op1 op2

imul3 :: FContext -> Word64 -> X86.Operand -> X86.Operand -> X86.Operand -> State Predicate ()
imul3 ctxt i_a op0 op1 op2 = do
  e1 <- read_operand ctxt op1
  e2 <- read_operand ctxt op2
  write_operand ctxt op0 (SE_Op Times (8 * sizeof op0) [e1,e2])
  write_flags (\_ _ -> None) op1 op2



bswap :: FContext -> Word64 -> X86.Operand -> State Predicate ()
bswap ctxt i_a op1 = do
  e1 <- read_operand ctxt op1
  write_operand ctxt op1 $ SE_Op Bswap (8 * sizeof op1) [e1]
  write_flags (\_ _ -> None) op1 op1

pextrb :: FContext -> Word64 -> X86.Operand -> X86.Operand -> X86.Operand -> State Predicate ()
...
pextrd :: FContext -> Word64 -> X86.Operand -> X86.Operand -> X86.Operand -> State Predicate ()
...
pextrq :: FContext -> Word64 -> X86.Operand -> X86.Operand -> X86.Operand -> State Predicate ()
...
xadd :: FContext -> Word64 -> X86.Operand -> X86.Operand -> State Predicate ()
xadd ctxt i_a op1 op2 = do
  e1 <- read_operand ctxt op1
  e2 <- read_operand ctxt op2
  write_operand ctxt op1 (SE_Op Plus (sizeof op1) [e1,e2])
  write_operand ctxt op2 e1
  write_flags (\_ _ -> None) op1 op2
...
tau_i :: FContext -> X86.Instruction -> State Predicate ()
...
tau_b :: FContext -> [X86.Instruction] -> State Predicate ()
tau_b ctxt []  = return ()
tau_b ctxt (i:is) = do
...
tau_block ::
  ...
  -> Predicate                -- ^ The predicate to be transformed
  -> (Predicate,S.Set VerificationCondition)
tau_block ctxt insts insts' p@(Predicate eqs flg) =
  if insts == [] then
    (p, S.empty)
  else let
      addr                   = X86.addressof $ head insts
      eqs'                   = write_rip addr eqs
      p''                    = execState (tau_b ctxt insts) (Predicate eqs' flg)
      Predicate eqs'' flgs'' = p'' in
    case insts' of
      Nothing -> (p'',S.empty)
      Just (i':_) ->
        let addr'  = X86.addressof i' in
          (Predicate (write_rip addr' eqs'') (add_jump_to_pred (last insts) i' flgs''), S.empty)
 where
...
get_invariant fctxt a = do
  ...
  return $ fst $ tau_block fctxt (takeWhile (\i -> fromIntegral (X86.addressof i) /= a) instrs) Nothing p



set_rip ctxt i = write_reg ctxt RIP (SE_Immediate $ X86.addressof i + (fromIntegral $ sizeof i))
...
try_deterministic fctxt v
  | contains_bot v = Nothing
  | otherwise      = Just v
