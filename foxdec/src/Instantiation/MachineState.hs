...
-- *  Registers
read_rreg :: Register -> State Predicate SimpleExpr
read_rreg r = do
  Predicate eqs flg <- get
  case M.lookup (SP_Reg r) eqs of
    Nothing -> do
      let var = SE_Var (SP_Reg r)
      put $ Predicate (M.insert (SP_Reg r) var eqs) flg
      return var 
    Just e  -> return e

-- | Read from a register
read_reg :: FContext -> Register -> State Predicate SimpleExpr
read_reg ctxt r = do
  let rr = Reg.real r
  v <- read_rreg rr
...
write_rreg :: FContext -> Register -> SimpleExpr -> State Predicate () 
write_rreg ctxt r e = 
...
-- | Write to a register
write_reg :: FContext -> Register -> SimpleExpr -> State Predicate ()
write_reg ctxt r v = do
 let sz  = sizeof r in
  if sz == 8 then -- 64 bit
    write_rreg ctxt r $ simp v
  else if sz == 4 then -- 32 bit
    write_rreg ctxt (Reg.real r) (simp $ SE_Bit 32 v)
  else if sz == 2 then do -- 16 bit 
    let rr = Reg.real r
    curr_v <- read_rreg rr
    write_rreg ctxt rr (simp $ SE_Overwrite 16 curr_v (SE_Bit 16 v))
  else if r `elem` Reg.reg8 then do -- 8 bit low registers
    let rr = Reg.real r
    curr_v <- read_rreg rr
    write_rreg ctxt rr (simp $ SE_Overwrite 8 curr_v (SE_Bit 8 v))
  else
    write_rreg ctxt (Reg.real r) $ Bottom (FromBitMode $ srcs_of_expr ctxt v)
...
resolve_address :: FContext -> X86.Address -> State Predicate SimpleExpr
...
is_assertable ctxt a0 si0 a1 si1 =
  let srcs0       = srcs_of_expr ctxt a0
    ...
    or [
      ...
      snd $ separate_pointer_domains ctxt a0 a1,
...
expr_to_mem_addresses ctxt a si =
  if not (contains_bot a) then
    ...
    let addresses = dom_to_mem_addresses $ fromJust $ get_pointer_domain ctxt a in
      addresses
 where 
...
read_from_address ctxt operand a si0 = do
 ...
 where
  ...
  read_from_state :: SimpleExpr -> State Predicate SimpleExpr
  ...
  is_separate :: SimpleExpr -> Int -> SimpleExpr -> Int -> State Predicate SeparationStatus
  is_separate a0 si0 a1 si1 = do
    if si0 == si1 && necessarily_equal a0 a1 then
      return Alias
    else if necessarily_separate_expressions a0 si0 a1 si1 then
      return Separated
  ...
  do_read :: SimpleExpr -> [(StatePart, SimpleExpr)] -> State Predicate SimpleExpr
  do_read a0 [] = do
    p <- get
    let Predicate eqs flg = p
    let sp  = SP_Mem a0 si0
    let var = Bottom $ FromUninitializedMemory $ S.singleton (Src_Var sp)
    put $ Predicate (M.insert sp var eqs) flg
    return var
...
write_mem ctxt a si0 v = do
 ...
 where
  ...
  do_write :: SimpleExpr -> Int -> SimpleExpr -> [(StatePart, SimpleExpr)] -> State Predicate [(StatePart, SimpleExpr)]
...
read_operand :: FContext -> X86.Operand -> State Predicate SimpleExpr
...
write_operand ::
 ...
 -> SimpleExpr           -- ^ The value to be written
 -> State Predicate ()
write_operand ctxt (Storage r)   v = write_reg ctxt r v
write_operand ctxt (Memory a si) v = do
  resolved_address <- resolve_address ctxt a
  write_mem ctxt resolved_address si v
...
read_sp :: FContext -> StatePart -> State Predicate SimpleExpr
...
write_sp :: 
  ...
  -> State Predicate ()
write_sp ctxt (sp@(SP_Reg r),v)    = write_reg ctxt r v
write_sp ctxt (sp@(SP_Mem a si),v) = write_mem ctxt a si v
